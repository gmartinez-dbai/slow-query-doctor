> **📢 Notice:** This RFC coincides with the rebranding of slow-query-doctor to Iqtoolkit Analyzer. 
...
## Rebranding: slow-query-doctor → Iqtoolkit Analyzer
...
SaaS database optimization tools require shipping production logs to third-party clouds—unacceptable for some:
- Healthcare (HIPAA)
...
- **PostgreSQL** (v0.2.2a1): EXPLAIN plan analysis, index recommendations, query rewriting, anti-pattern detection
- **MongoDB** (v0.2.2a1): Aggregation pipeline optimization, collection slow query detection, index strategy recommendations, document structure analysis
...
- Package restructuring from slow-query-doctor → iqtoolkit-analyzer
...
**Design rationale:** Enterprises must choose between privacy (local Ollama) and capability (cloud OpenAI). No competitor offers this choice—most force SaaS.
...
**Rationale:**
- Sequential development (PostgreSQL → MongoDB) delays MongoDB support by 6-12 months
- Building both simultaneously validates the multi-database framework architecture early
- Many of enterprises run both databases—unified intelligence is the market need[^1]
...
**Trade-offs:**
- Community confusion (old name → new name)
...
## Roadmap: v0.2.2a1 → v1.0 GA

| Version | Priority | Databases | Key Features | Status |
|---------|----------|-----------|--------------|--------|
| **v0.2.2a1** | Current | PostgreSQL + MongoDB | Core analyzers, OpenAI integration, multi-format reports, package migration | 🚧 **Migration in Progress** |
| **v0.2.3** | Next | PostgreSQL + MongoDB | Ollama integration (privacy-first AI), MongoDB pipeline optimizer, Docker deployment | 🚧 **In Progress** |
| **v0.3.0** | High | PostgreSQL + MongoDB | ML anomaly detection, developer workflow integrations | 📋 **Planned** |
| **v0.4.0** | Medium | + MySQL | MySQL slow query analyzer, unified optimization framework | 📋 **Planned** |
| **v0.5.0** | Medium | + SQL Server | SQL Server query optimization, full multi-database support | 📋 **Planned** |
| **v1.0 GA** | Future | All 4 databases | Production-ready enterprise deployment for all databases | 📋 **Planned** |
...
- MongoDB slow query detection with profiler integration
- OpenAI integration for AI recommendations

### 🚧 Critical Gaps (Blocking "Open-Source Standard" Vision)
...
3. **MongoDB aggregation pipeline analyzer** (planned v0.2.3) - differentiator vs. MongoDB Compass
4. **GitHub Actions integration** (planned v0.3.0) - embed in developer workflow
5. **Release automation** (Issue #34) - velocity and consistency
...
### Rebranding
6. **Does the rebrand from slow-query-doctor → Iqtoolkit Analyzer make sense?**
...
### Submit Feedback
- **GitHub Discussions:** [Reply to RFC-001](https://github.com/iqtoolkit/iqtoolkit-analyzer/discussions)
...
## Migration Guide: slow-query-doctor → Iqtoolkit Analyzer
