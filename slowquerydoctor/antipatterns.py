        ...
        for pattern_type, config in self.patterns.items():
            ...
            for match in regex_matches:
                anti_pattern = AntiPatternMatch(
                    ...
                    confidence_score=self._calculate_confidence(pattern_type, match, query)
                )
                matches.append(anti_pattern)
        
        return matches
    
    def _calculate_confidence(self, pattern_type: AntiPatternType, 
                            match: re.Match, query: str) -> float:
        ...
        if pattern_type == AntiPatternType.LARGE_IN_CLAUSE:
            ...
            like_content = re.search(r"LIKE\s+['\"]([^'\"]+)['\"]", match.group(), re.IGNORECASE)
            if like_content and like_content.group(1).startswith('%'):
                return base_confidence + 0.1
        
        return base_confidence
    
    def generate_rewrite_report(self, query: str, matches: List[AntiPatternMatch]) -> str:
        """
